...
const KDS_APPS = [
  {
    id: 'workspace',
    name: 'KDS Workspace',
    url: 'https://workspace.kierendaystudios.co.uk/',
    icon: 'workspace',
    category: 'Productivity',
    description: 'Docs, slides, spreadsheets, notes and project management.',
    color: '#3b82f6',
  ...
  },
  {
    id: 'retbuild',
    name: 'Retbuild',
    url: 'https://retbuild.co.uk/',
    icon: 'retbuild',
    category: 'Productivity',
    description: "Build micro apps, software prototypes and ai agents with Google's Gemini.",
    color: '#10b981',
    gradient: 'linear-gradient(135deg, #10b981, #059669)'
  },
  {
    id: 'founders',
    name: 'KDS Founders OS',
    url: 'https://founders.kierendaystudios.co.uk/',
    icon: 'founders',
    category: 'Productivity',
    description: 'Manage business projects, ideas, links, tasks, roadmaps and more.',
    color: '#f59e0b',
  ...
  },
  {
    id: 'code',
    name: 'KDS Code',
    url: 'https://codestudio.kierendaystudios.co.uk/',
    icon: 'code',
    category: 'Development',
    description: 'Modern sleek IDE for creating web based applications and platforms.',
    color: '#0ea5e9',
    gradient: 'linear-gradient(135deg, #0ea5e9, #0284c7)'
  },
  {
    id: 'academy',
    name: 'KDS Web Academy',
    url: 'https://academy.kierendaystudios.co.uk/',
    icon: 'academy',
    category: 'Development',
    description: 'Learn how to build websites in HTML, CSS, and JS with a built-in IDE.',
    color: '#ef4444',
  ...
  },
  {
    ...
    url: 'https://gamedev.kierendaystudios.co.uk/#/dashboard',
    icon: 'gamedev',
    category: 'Development',
    description: 'Indie gaming platform by KDS.',
    color: '#8b5cf6',
    gradient: 'linear-gradient(135deg, #8b5cf6, #7c3aed)'
  },
  {
    id: 'stock',
    name: 'KDS Stock Images',
    url: 'https://stock.kierendaystudios.co.uk/',
    icon: 'stock',
    category: 'Resources',
    description: 'Commercially free to use stock images.',
    color: '#6b7280',
  ...
  },
  {
    id: 'gaming',
    name: 'KDS Gaming',
    url: 'https://gaming.kierendaystudios.co.uk/#/dashboard',
    icon: 'gaming',
    category: 'Resources',
    description: 'Indie gaming platform by KDS.',
    color: '#ec4899',
...
export const Desktop = () => {
  ...
  useEffect(() => {
    ...
    localStorage.setItem(
      getStorageKey(userId, 'cordoval-settings'),
      JSON.stringify({ wallpaper, accentColor, zoom })
    )
    try {
  ...
  useEffect(() => {
    localStorage.setItem(
      getStorageKey(userId, 'cordoval-icon-positions'),
  ...
  useEffect(() => {
    localStorage.setItem(
      getStorageKey(userId, 'cordoval-installed-apps'),
  ...
  const openKDSBrowser = (initialUrl?: string) => {
    ...
    if (windows.find((w) => w.id === browserId)) {
  ...
  const openFileExplorer = () => {
    ...
    if (windows.find((w) => w.id === explorerId)) {
      focusWindow(explorerId)
      return
    }
  ...
  const openSettings = () => {
    ...
    if (windows.find((w) => w.id === settingsId)) {
    ...
    >
      {showUpdatePrompt && updateInfo && (
        ...
        >
          ...
          {updateInfo.status === 'available' && (
            <>
              <p style={{ fontSize: 13, marginBottom: 15 }}>
                A new version (v{updateInfo.version}) is available. Would you like to download it?
              </p>
              <button
                onClick={(e) => {
                  e.stopPropagation()
                  handleUpdateAction('download')
                }}
                style={{
                  width: '100%',
                  padding: '8px',
          ...
          {updateInfo.status === 'downloading' && (
            <>
              <p style={{ fontSize: 12, marginBottom: 10 }}>
                Downloading update... {Math.round(updateInfo.percent || 0)}%
              </p>
              <div
                style={{
                  width: '100%',
          ...
          {updateInfo.status === 'downloaded' && (
            <>
              <p style={{ fontSize: 13, marginBottom: 15 }}>
                Update v{updateInfo.version} is ready to install. The system will restart.
              </p>
              <button
                onClick={(e) => {
                  e.stopPropagation()
                  handleUpdateAction('install')
                }}
                style={{
                  width: '100%',
                  padding: '8px',
      ...
      >
        ...
        {contextMenu && (
          ...
          >
            ...
            <button onClick={() => handleRemoveFromDesktop(contextMenu.itemId)}>
              Remove from Desktop
            </button>
          </div>
        )}
        ...
        <div className="start-menu" onClick={(e) => e.stopPropagation()}>
          <div className="start-menu-content">
            ...
            <div className="start-menu-scroll">
              ...
              <div className="start-apps-grid-title" style={{ marginTop: 25 }}>
                KDS Ecosystem
              </div>
              <div className="start-apps-grid">
                {KDS_APPS.map((app) => (
                  ...
                  <div className="start-apps-grid">
                    {getInstalledStoreApps().map((app) => (
                      <StartAppItem
                        ...
                        name={app.name}
                        icon={app.icon}
                        onClick={() => handleOpenStoreApp(app)}
                      />
                    ))}
                  </div>
                </>
                  ...
                  <div className="start-apps-grid">
                    {hostApps.map((app) => (
                      <StartAppItem
                        ...
                        name={app.name}
                        icon="🪟"
                        onClick={() => launchHostApp(app.path)}
                      />
                    ))}
                  </div>
                </>
      ...
      <div className="taskbar" onClick={(e) => e.stopPropagation()}>
        ...
        {windows.map((win) => (
          <div
            key={win.id}
            className={`taskbar-icon ${activeWindowId === win.id && !win.isMinimized ? 'active' : ''}`}
        ...
        <div className="system-tray">
          <span
            ...
            style={{ cursor: 'help', opacity: 0.8, display: 'flex', alignItems: 'center' }}
          >
...
const GeneratedApp = ({ code }: { code: string }) => (
  <div style={{ width: '100%', height: '100%', background: 'white' }}>
    <iframe
      srcDoc={code}
      title="Noah Generated App"
    ...
    >
      ...
      <div className="window-titlebar" onMouseDown={handleMouseDown} onDoubleClick={onMaximize}>
        ...
        <div className="window-controls" onMouseDown={(e) => e.stopPropagation()}>
          ...
          <div className="control-btn close" onClick={onClose}>
            ✕
          </div>
        </div>
      </div>
...
const CalendarWidget = () => {
  ...
  return (
    ...
    >
      ...
      <h2 style={{ margin: '0 0 15px 0', fontSize: 18, fontWeight: 800 }}>
        {dayNumeric} {now.toLocaleDateString('en-US', { month: 'long' })} {now.getFullYear()}
      </h2>
      <div
        style={{
          opacity: 0.6,
      ...
      >
        {['Su', 'Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa'].map((d) => (
          <div key={d} style={{ color: 'rgba(0,0,0,0.3)' }}>
            {d}
          </div>
        ))}
      ...
      <div style={{ marginTop: 15, paddingTop: 15, borderTop: '1px solid rgba(0,0,0,0.05)' }}>
        <div style={{ fontSize: 11, fontWeight: 700, color: 'rgba(0,0,0,0.4)', marginBottom: 8 }}>
          TODAY
        </div>
        <div
          style={{
        ...
        >
          <div
            style={{ width: 4, height: 14, background: 'var(--accent-color)', borderRadius: 2 }}
          />
          No events scheduled
        </div>
      </div>
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <span style={{ fontSize: 13, fontWeight: 800, letterSpacing: 0.5, opacity: 0.6 }}>
          QUICK NOTES
        </span>
      </div>
      <textarea
